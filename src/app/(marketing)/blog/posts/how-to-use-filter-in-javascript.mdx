---
title: 'How to use filter in JavaScript - A Complete Guide with Examples'
description: "Master JavaScript's filter method with coding challenges, step-by-step explanations, and real-world use cases. Learn how to efficiently filter arrays to get exactly the data you need."
...
<CodeSnippet language="javascript" content={`const numbers = [1, 2, 3, 4, 5, 6];
...
<CodeSnippet language="javascript" content={`Array.filter(function(currentValue, index, arr), thisValue)`} />
...
<CodeSnippet language="javascript" content={`// Filter products within a specific price range
const products = [
...
<CodeSnippet language="javascript" content={`// Filter users who are old enough to vote
const users = [
  { name: 'Alice', age: 25 },
  { name: 'Bob', age: 17 },
  { name: 'Charlie', age: 21 },
...
<CodeSnippet language="javascript" content={`// Start with an array of user objects
const users = [
  { name: 'Alice', age: 25 },
  { name: 'Bob', age: 17 },
  { name: 'Charlie', age: 21 },
...
<CodeSnippet language="javascript" content={`const canVote = users.filter(user => user.age >= 18);`} />
...
<CodeSnippet language="javascript" content={`// Filter active premium users
const users = [
...
<CodeSnippet language="javascript" content={`// Array of users with premium and active status
const users = [
  { name: 'Alice', isPremium: true, isActive: true },
  { name: 'Bob', isPremium: false, isActive: true },
  { name: 'Charlie', isPremium: true, isActive: false },
...
<CodeSnippet language="javascript" content={`const activePremiumUsers = users.filter(user => user.isPremium && user.isActive);`} />
...
<CodeSnippet language="javascript" content={`// Filter unique values from an array
...
<CodeSnippet language="javascript" content={`const unique = numbers.filter((value, index, array) => array.indexOf(value) === index);`} />
...
<CodeSnippet language="javascript" content={`// Remove falsy values from an array
const mixed = [0, 1, false, true, '', 'hello', null, undefined, NaN];

const truthyOnly = mixed.filter(Boolean);
console.log(truthyOnly); // [1, true, 'hello']`} />
...
<CodeSnippet language="javascript" content={`const mixed = [0, 1, false, true, '', 'hello', null, undefined, NaN];`} />

<br />

2. We use the Boolean function as our filter callback:
<CodeSnippet language="javascript" content={`const truthyOnly = mixed.filter(Boolean);`} />
