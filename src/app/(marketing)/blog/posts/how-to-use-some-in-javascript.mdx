...
<CodeSnippet language="javascript" content={`Array.some(callback(element[, index[, array]])[, thisArg])`} />
...
<CodeSnippet language="javascript" content={`// Checking for even numbers
...
<CodeSnippet language="javascript" content={`// Checking for a specific string
...
<CodeSnippet language="javascript" content={`// Checking objects for a property
const people = [
  { name: 'Alice', age: 25 },
  { name: 'Bob', age: 30 },
  { name: 'Charlie', age: 20 }
...
console.log(hasPersonUnder25); // true`} />
