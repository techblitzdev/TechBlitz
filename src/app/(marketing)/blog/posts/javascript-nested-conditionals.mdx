...
<CodeSnippet language="javascript" content={`if (isLoggedIn) {
  if (hasAdminRights) {
    if (isSuperAdmin) {
...
<CodeSnippet language="javascript" content={`function validatePurchase(user, item, cart) {
...
<CodeSnippet language="javascript" content={`// Bad: Deep nesting
if (condition1) {
  if (condition2) {
    if (condition3) {
      if (condition4) {
...
<CodeSnippet language="javascript" content={`// Before
...
<CodeSnippet language="javascript" content={`// Instead of nested ifs
const canAccess = isLoggedIn && hasPermission && isActive;

// Use with nullish coalescing
const userRole = user?.settings?.role ?? 'guest';`} />
...
<CodeSnippet language="javascript" content={`// Before refactoring
...
<CodeSnippet language="javascript" content={`const userActions = {
  admin: () => handleAdmin(),
...
const handleUser = (userType) => userActions[userType]?.() ?? handleGuest();`} />

### 2. State Machines

For complex state management:

<CodeSnippet language="javascript" content={`const orderStates = {
  pending: {
    canCancel: true,
    canShip: false,
    next: 'processing'
