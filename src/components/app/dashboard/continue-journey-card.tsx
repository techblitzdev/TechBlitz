...
export default async function ContinueJourneyCard() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
      ...
      <Link
        href={`/questions/study-paths/${studyPaths?.[0]?.studyPath.slug}`}
        style={{
          background:
            'radial-gradient(128% 107% at 0% 0%,#212121 0%,rgb(0,0,0) 77.61472409909909%)',
        }}
          ...
          <div className="flex w-full items-center gap-x-2 justify-between">
            ...
            <div className="flex flex-col gap-y-2 w-full">
              <p className="text-sm text-gray-400 font-onest">
                ...
                {Math.round(studyPaths?.[0]?.progress ?? 0) === 100 ? (
                  ...
                  `${Math.round(studyPaths?.[0]?.progress ?? 0)}% completed`
                )}
              </p>
              <Progress
                className="border border-black-50 bg-black-50"
                value={studyPaths?.[0]?.progress ?? 0}
              />
            </div>
          </div>
        </div>
