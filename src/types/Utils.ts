...
export type RequireAtLeastOne<T> = {
  ...
  [K in keyof T]-?: Required<Pick<T, K>> & // Example: if K is 'name', this makes { name: string } required // Required<...> makes that property required // Pick<T, K> selects just the current property K from T
    // Part 1: Required<Pick<T, K>> // For each property K, we create a type that:
